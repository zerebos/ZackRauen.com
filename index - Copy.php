...
$project = count(explode('/',"$page/")) > 2 ? explode('/',"$page/")[2] : NULL;
$project = str_replace("/","",$project);
// $project = count(explode('/',"$page/")) > 3 ? explode('/',"$page/")[3] : NULL;
		...
		<nav class="collapse accent-font">
			...
			<ul class="mainNav">
				...
				<li>
					...
					<ul>
						<li>
							<a href='/blog'>Poetry</a>
						</li>
						<li>
							<a href='/blog'>Life Stories</a>
						</li>
						<li>
							<a href='/blog'>Blog</a>
						</li>
...
if (strpos($page,"/Projects")!==true && strpos($page,"/Projects")===0) {
include_once($home."/resources/Parsedown.php");
$parser = new Parsedown();
if (!$project || !file_exists($home."/Projects/".$project)) {
	$files = scandir($home."/Projects");

	foreach ($files as $file) {
		if ($file != "." && $file != "..") {
			if (is_dir($home."/Projects/".$file)) {
				$filename = $home."/Projects/$file/LISTING.md";
				$handle = fopen($filename, "r");
				$contents = fread($handle, filesize($filename));
				$parsed=$parser->text($contents);

				if (file_exists($home."/Projects/".$file."/thumbnail.png")) {
				$image = "/Projects/".$file."/thumbnail.png";
				} else {
				$image = "/images/defaultProject.jpg";
				}
				$parsed=str_replace("</h2>","</h2><img src=\"".$image."\" alt=\"Project image\" class=\"projectImg\" />",$parsed);
				?>
					<div class="card minimizable">
						<div class="card-header">
							<h1>
								<?php echo $file;?> Description</h1>
...
if (!file_exists($home.$page)) {
$page = "/Projects/".$category."/".$project."/";
}
	listFiles($home,$page,$project);
	echo "<div class=\"card minimizable\">";
	echo '<div class="card-header">
	<h1>'.$project.' Description</h1>
	<span class="fa fa-minus button-minimize"></span>
	</div><div class="content-container">';
	$filename = $home."/Projects/".$category."/".$project."/README.md";
	$handle = fopen($filename, "r");
	$contents = fread($handle, filesize($filename));
	$parsed=$parser->text($contents);
	$parsed=str_replace("src=\"","src=\"/Projects/".$category."/".$project."/",$parsed);
	echo $parsed;
	fclose($handle);
	echo "</div></div>";
}
}
elseif (strpos($page,"/PublicFiles")!==true && strpos($page,"/PublicFiles")===0) {
if (!file_exists($home.$page)) {
$page = "/PublicFiles/";
}
listFiles($home,$page,"Public Files");
}
elseif (strpos($page,"/Contact")!==true && strpos($page,"/Contact")===0) {
include_once("resources/contact.php");
}
elseif (strpos($page,"/BlockCatcher")!==true && strpos($page,"/BlockCatcher")===0) {
...
elseif (strpos($page,"/About")!==true && strpos($page,"/About")===0) {
...
elseif (strpos($page,"/Debug")!==true && strpos($page,"/Debug")===0) {
...
<div class="card project minimizable">
	...
	<div class="card-footer content-container">
		...
		<a href="<?= $repo->html_url ?>" target="_blank" class="footer-link pull-right">
			<img src="/images/svg/github.svg" class="footer-icon" width="14" height="16" alt="GitHub" />
			View on GitHub
		</a>
	</div>
</div>
...
?>
								...
								<div class="card minimizable">
									...
									<div class="content-container">
										<span class="important">ATTENTION:</span> This site is still being developed, the IA has just been introduced to its final phase.
										<br />
										<br /> Congrats! You made it to, what is possibly, the greatest website in the world. This page holds news and updates
										for the site, so if you're looking for something else, it's probably on another page. Check the navigation above
										to see if what you want is there!
									</div>
								</div>
...
?>
			...
			<section class="width-1-of-4">
				<aside class="card minimizable">
					...
					<figure class="content-container icons">
					...
					<ul class="inline-list">
						<li>
							<a href="/Contact" target="_blank">
								<span class="fa fa-fw fa-envelope-square"></span>Email</a>
						</li>
						<li>
							<a href="http://facebook.com/ZackRauen" target="_blank">
								<span class="fa fa-fw fa-facebook-official"></span>Facebook</a>
						</li>
						<li>
							<a href="http://twitter.com/ZackRauen" target="_blank">
								<span class="fa fa-fw fa-twitter-square"></span>Twitter</a>
						</li>
						<li>
							<a href="http://instagram.com/ZackRauen" target="_blank">
								<span class="fa fa-fw fa-instagram"></span>Instagram</a>
						</li>
						<li>
							<a href="http://github.com/rauenzi" target="_blank">
								<span class="fa fa-fw fa-github-square"></span>GitHub</a>
						</li>
						<li>
							<a href="http://linkedin.com/in/rauenzi" target="_blank">
								<span class="fa fa-fw fa-linkedin-square"></span>LinkedIn</a>
						</li>
					</ul>
				</section>
				<section class="width-full">
					All content attributed to Zachary Rauen. Icons provided by Font-Awesome.
					<span id="toTop" style="cursor: pointer;">Back To Top</span>
				</section>
			</div>
		</footer>
			...
			$(function () {
				$(".button-minimize").on("click", function (e) {
					if ($(this).hasClass("fa-plus")) {
						$(this).closest(".minimizable").find(".content-container").slideDown();
					} else {
						$(this).closest(".minimizable").find(".content-container").slideUp();
					}
